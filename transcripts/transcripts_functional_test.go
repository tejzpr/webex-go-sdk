...
// TestFunctionalTranscriptsCursorNavigation tests PageFromCursor with transcripts
// Run with:
//
//	WEBEX_ACCESS_TOKEN=<your-token> go test -tags functional -run TestFunctionalTranscriptsCursorNavigation -v ./transcripts/
func TestFunctionalTranscriptsCursorNavigation(t *testing.T) {
	token := os.Getenv("WEBEX_ACCESS_TOKEN")
	if token == "" {
		t.Fatal("WEBEX_ACCESS_TOKEN environment variable is required")
	}
